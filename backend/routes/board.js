...
router.post("/handleWrite", (req, res) => {
  ...
  conn.query(
    sql,
    [userId, boardType, boardTitle, boardContent, boardLike],
    (err, results) => {
      if (err) throw err;
...
router.get("/getPosts", (req, res) => {
  ...
  conn.query(sql, sqlParams, (err, results) => {
    ...
    // 게시물 총 개수 계산
    const countSql =
      "SELECT COUNT(*) AS totalCount FROM boardInfo" +
      (type > 0 ? " WHERE board_type = ?" : "");
...
router.put("/posts/:board_id", (req, res) => {
  ...
  conn.query(
    "SELECT user_id FROM boardInfo WHERE board_id = ?",
    [board_id],
    (err, results) => {
      if (err) throw err;
      ...
      if (results.length === 0) {
        ...
        conn.query(
          "UPDATE boardInfo SET board_content = ? WHERE board_id = ?",
...
router.delete("/posts/:board_id", (req, res) => {
  ...
  conn.query(
    ...
    (err, results) => {
      // 댓글 삭제
      conn.query(
        "DELETE FROM commentInfo WHERE board_id = ?",
        [board_id],
        (err) => {
          if (err)
            return res
              .status(500)
              .json({ success: false, message: "Failed to delete comments" });

          // 게시글 삭제
          conn.query(
            "DELETE FROM boardInfo WHERE board_id = ?",
            [board_id],
            (err) => {
              if (err)
                return res
